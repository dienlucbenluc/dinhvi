...
function loadChiSoData() {
  ...
  .catch(() => {
    document.getElementById("listContainer").innerHTML = "<p style='color:red; text-align:center;'>L?i k?t n?i máy ch?!</p>";
  });
}
...
async function getLocation(maKhang) {
  ...
  if (hasCoords) {
    const confirmAgain = await showCustomConfirm("L?Y T?A Ð? GPS", `B?n có mu?n l?y l?i t?a d? m?i cho mã khách hàng ${maKhang} này không?`);
    if (!confirmAgain) return;
  }

  if (navigator.geolocation) {
    showToast(`? Ðang l?y v? trí GPS...`);
    navigator.geolocation.getCurrentPosition(
      (position) => {
        ...
        .then(res => {
          if (res.status === "success") {
            showToast("?? Ðã l?y & luu t?a d? thành công!");
        ...
        .catch(() => showToast("? L?i k?t n?i máy ch?!"));
      },
      (error) => { showToast("? Không th? l?y GPS! B?t v? trí thi?t b?."); },
      { enableHighAccuracy: true, timeout: 10000 }
    );
  } else {
    showToast("? Thi?t b? không h? tr? GPS!");
  }
}
...
function renderGroupedList(groups) {
  ...
  if (keys.length === 0) {
    container.innerHTML = "<p style='text-align:center; padding-top:20px;'>Không có d? li?u khách hàng.</p>";
    return;
  }
  ...
  keys.forEach(makh => {
    ...
    let mapLinkHtml = `<span id="map_link_${cust.ma_khang}" style="color:#dc3545; font-weight:bold;">?? Chua có t?a d?</span>`;
    if (hasLocation) {
    ...
    html += `
      <div class="customer-card ${isActive}" id="card_${cust.ma_khang}" onclick="selectCustomer('${cust.ma_khang}')">
        <div class="cust-header">
          ...
          <div class="cust-address">Ð?a ch?: ${cust.dia_chi || ''}</div>
          ...
          <div class="cust-row-group" style="margin-top: 3px;">
            ...
            <input type="text" 
                   ...
                   value="${cust.ghi_chu || ''}" 
                   placeholder="Nh?p, s?a ghi chú n?u có..." 
    ...
    html += `
        ...
        <div class="card-btn-group">
          <button class="btn-card btn-card-location" onclick="event.stopPropagation(); getLocation('${cust.ma_khang}')">Ð?NH V?</button>
...
async function saveCustomerData(maKhang) {
  ...
  if (isWarning) {
    confirmMessage = warningMsgs.join("\n") + "\n?Ki?m tra ch? s? trên công to k? l?i.\n?? B?n v?n mu?n xác nh?n ghi d? li?u?";
  } else {
    confirmMessage = "Xác nh?n ghi d? li?u ch? s? và ghi chú?";
  }

  const confirmSave = await showCustomConfirm(
    isWarning ? "C?NH BÁO S?N LU?NG" : "XÁC NH?N GHI D? LI?U",
    confirmMessage,
    isWarning
  );
  ...
  showToast(`? Ðang luu d? li?u KH ${maKhang}...`);
  fetch(API_URL, {
    method: "POST",
    headers: { "Content-Type": "text/plain;charset=utf-8" },
    body: JSON.stringify({
...
async function cancelCustomerData(maKhang) {
  ...
  const confirmCancel = await showCustomConfirm(
    "XÁC NH?N H?Y", 
    `B?n có ch?c ch?n mu?n H?Y ch? s? dã nh?p c?a khách hàng ${maKhang}? (Ghi chú và tr?ng thái nh?p CMIS s? gi? nguyên)`,
    true
  );
  ...
  showToast(`? Ðang h?y ch? s? KH ${maKhang}...`);
  fetch(API_URL, {
    method: "POST",
    headers: { "Content-Type": "text/plain;charset=utf-8" },
    body: JSON.stringify({
